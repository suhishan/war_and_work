...
districts_joined <- districts_joined %>% mutate(
  ...
  cd_per_10000 = round(cd_per_10000, 2)
)
district_centroids <- districts_joined %>%
  st_point_on_surface() %>%   
  st_coordinates() %>%
  as.data.frame() %>%
  cbind(districts_joined)
...
p2 <- ggplot(districts_joined) +
  ...
  labs(x = "", y = "", 
       title = "Treatment Classification into High-and-Low Conflict Districts")+
  geom_label_repel(
    data = district_centroids,
    aes(X, Y, label = paste(distname, as.character(cd_per_10000))),
    size = 4,
    box.padding = 0.3,   
    point.padding = 0.3, 
    segment.color = NA
...
ggsave("Analysis files/overall_conflict_map.png", plot = p2,
       width = 45, height = 30, units = c("cm"), bg = "white")
...
[] Remember to do exp(mu + (sigma^2)/2)
...
coefplot_map <- post_log_2 %>%
  ...
  labs( y = "", x = "", subtitle = "Work Hours conditional on Work Hours > 0")+
...
```{r}
# function that takes in E(log(x)) and SD(log(x)) and returns E(x)
mu_ln <- function(mu, sigma) {
...
post_log_2 %>% 
  ...
  mutate(
    did_a = (mu_ln(b_group4, sigma) - mu_ln(b_group2, sigma)) -
      (mu_ln(b_group3, sigma) - mu_ln(b_group1, sigma))
  ) %>% 
  pull(did_a) %>% 
  mean_qi()
```
...
post_log_2 %>% 
  select(contains("b_hu")) %>% 
  mutate(did_r = (b_hu_group4 - b_hu_group2) - 
           (b_hu_group3 - b_hu_group1)) %>% 
  mutate(
...
### Plot of prior vs posterior comparison.
