...
NOTE: Both NLFS 1 and NLFS 2 don't have data on Manang and Humla
...
# Seasons
d1 <- d1 %>% mutate(
  season = factor(season.x, levels = c(1, 2, 3),
                  labels = c("Rainy", "Winter", "Dry"))
)
...
```{r}
d1_select <- d1 %>% select(nlfs_year, psu, age, sex, hhsize, urbrur, season, perma_resident,
...
d2 <- d2 %>% mutate(
  ...
  season = factor(season, levels = c(2, 3, 1),
                  labels = c("Rainy", "Winter", "Dry"))
)
...
```{r}
d2_select <- d2 %>% select(nlfs_year, psu, age, sex, hhsize, urbrur,season, perma_resident,
                           marital_status, hindu, brahmin_chhetri,
                           district_name, district_abbrev, 
                           wage_hours, selfemp_hours, selfemp_hours_outside,
...
### Merge with Do and Iyer dataset.

```{r}
do <- read_dta("Data/forest.dta")
do <- do %>% mutate(
  district_abbrev = case_when(
    distname == "Dhankuta" ~ "dhak",
    distname == "Sindhupalchok" ~ "sinp",
    TRUE ~ str_sub(str_to_lower(distname),1, 4)
  )
)

# Let's try and merged it with c_merged.

c_collapsed_2 <- left_join(c_collapsed_2, do, by = c("district_abbrev"))

```


### Only those districts with less than 0 deaths by 1998.
