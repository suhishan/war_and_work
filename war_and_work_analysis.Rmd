...
```{r}
model_1_out <- model_to_tex(model_1, "Overall", "model_1")
```

#### Employed/Unemployed Statuses for Coefficient Plots
...
```{r}
model_2_out <- model_to_tex(model_2, "Accounting for migration", "model_2")
```

###  Bernoulli Employed Variables for Coefficient Plots.

```{r}
# Coefficient Plots for the first three Bernoulli variables.

model_2_1 <- model_2_out[1:3, ]
model_2_1$Outcome <- c("Usually Employed (m)",
                         "Currently Employed (m)",
                         "Currently Self Employed (m)")
```

#### Work Hours for the Migration Model.

```{r}
model_2_2 <- model_2_out[c(4, 6),]
model_2_2$Outcomes <- c("Work Hours (m)", "Self Employment Hours (m)")

```

### Coef Plot for Bernoulli Employed variables.
...
ggsave("Analysis files/coefplot_1.jpg", plot = coefplot_1 ,
       width = 35, height = 15, units = c("cm"), bg = "white")

```

### Work Hours Coefficient Plot.
...
ggsave("Analysis files/coefplot_2.jpg", plot = coefplot_2 ,
       width = 35, height = 15, units = c("cm"), bg = "white")
...
Dry : Magh, Falgun, Chaitra, Baisakh.
Winter : Ashoj Kartik Mangsir Poush
Rainy :Jestha, Ashaar, Saaun, Bhadra

```{r}
c_merged_dry <- c_merged %>% filter(season == "Dry")
c_merged_rainy <- c_merged %>% filter(season == "Rainy")
c_merged_winter <- c_merged %>% filter(season == "Winter")
...
```{r}
# Model Dry
model_dry <- output_dr(vars, c_merged_dry) %>% 
  rownames_to_column(var = "variables")

# Model Rainy
model_rainy <- output_dr(vars, c_merged_rainy) %>% 
  rownames_to_column(var = "variables")

# Rainy season has a humongous negative effect.
# Winter season actually has a positive effect.

# Model Winter
model_winter <- output_dr(vars, c_merged_winter) %>% 
  rownames_to_column(var = "variables")
...
model_to_tex(model_rainy, "Rainy Season", "model_rainy")
model_to_tex(model_winter, "Winter Season", "model_winter")
```
