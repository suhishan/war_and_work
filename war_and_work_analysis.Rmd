...
print(xtable(t1_out, caption = "Covariate Balance Statistics for trends/differences (2008 - 1998)"), 
      type = "latex",
      file = "Analysis files/covariate_balance_trends_41.tex",
      booktabs = T,
      )
t1_out
...
ps <- glm(treatment_s ~  age + hindu + brahmin_chhetri + 
          sex + years_of_edu_all + poverty_rate + urbrur,
...
output_ip <- function(vars, d) {
  b <- sapply(vars, ip, d)
  return (data.frame(t(b)))
}

model_1 <- output_ip(vars, c_merged) %>% 
...
model_2 <- output_ip(vars, c_merged_p) %>% 
  rownames_to_column(var = "variables")
